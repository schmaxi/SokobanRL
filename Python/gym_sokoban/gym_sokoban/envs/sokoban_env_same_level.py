...
class SokobanEnvSameLevel(SokobanEnv):
    metadata = {
        'render.modes': ['human', 'rgb_array', 'tiny_human', 'tiny_rgb_array', 'raw', 'greyscale'],
        'render_modes': ['human', 'rgb_array', 'tiny_human', 'tiny_rgb_array', 'raw', 'greyscale']
    }
    ...
    def __init__(self,
        ...
        self.reward_last = 0
        self.reward_dead_end = -100
        self.reward_no_op = -0.5
        ...
        if "greyscale" in self.render_mode:
            self.observation_space = Box(low=0, high=255, shape=(screen_height, screen_width), dtype=np.uint8)
        else:
            self.observation_space = Box(low=0, high=255, shape=(screen_height, screen_width), dtype=np.uint8)
    ...
    #     self.updateCustoms(info)
        
    #     return observation, self.reward_last, done,self._get_info(info)

    # def updateCustoms(self, info):
    #     # Custom Variables
    ...
    #         self.done_cnt += 1
    #     self.reward_current_episode += self.reward_last
    ...
    def reset(self, seed=None, options= {"second_player":False,
        ...
        #     try:
        #         self.room_fixed_start, self.room_state_start, self.box_mapping_start = generate_room(
        #             dim=self.dim_room,
    ...
    #     else:
    #         # img = room_to_rgb(self.room_state, self.room_fixed) // TODO: uncomment for normal size
    #         img = room_to_tiny_world_rgb(self.room_state, self.room_fixed, scale=scale)
    ...
    #     info["no_action_cnt"] = self.no_action
    #     info["player_moved_cnt"] = self.moved_cnt
    #     info["pushedBox_cnt"] = self.pushedBox_cnt
    #     info["done_cnt"] = self.done_cnt
    #     info["reward_current_episode"] = self.reward_current_episode
    #     return info
...
RENDERING_MODES = ['rgb_array', 'human', 'tiny_rgb_array', 'tiny_human', 'raw', 'greyscale']
